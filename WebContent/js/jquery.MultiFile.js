...
if (window.jQuery)(function ($) {
		...
		this
			...
			.each(function () {
				...
				if (!MultiFile.list) {
					...
					//multifiles.list.append('<div class="MultiFile-list" id="' + MultiFile.wrapID + '_list"></div>');
					//multifiles.list.list = $('#' + multifiles.wrapID + '_list');
				};
				MultiFile.list = $(MultiFile.list);
				//multifiles.list.list = $(multifiles-list.list);
				//===
								...
								label =	$(
										(
											'<span class="MultiFile-label" title="' + t + '">'+
												'<span class="MultiFile-title file-name">'+ n +'</span>'+
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		STRING: {
			remove: 'x',
			denied: 'jpg,png,bmp,ppt,xls만 업로드 가능합니다.\n다른 파일로 다시 시도해 주십시오...',
			file: '$file',
